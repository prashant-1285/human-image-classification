...
class TrainingPlot(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.savefig("/content/drive/MyDrive/VGG16/loss.jpg")
        if epoch==25:
            plt.savefig("/content/drive/MyDrive/VGG16/loss_{}.jpg".format(epoch))
        if epoch==50:
            plt.savefig("/content/drive/MyDrive/VGG16/loss_{}.jpg".format(epoch))
        if epoch==75:
            plt.savefig("/content/drive/MyDrive/VGG16/loss_{}.jpg".format(epoch))
        ...
        print("accuracies",self.acc,self.val_acc)
        plt.plot(N, self.val_acc, label = "validation_accuracy")
        plt.title("Training and Validation Accuracy")
        plt.xlabel("Epoch")
        plt.ylabel("Accuracy")
        plt.legend()
        plt.savefig("/content/drive/MyDrive/VGG16/accuracy.jpg")
        if epoch==25:
            plt.savefig("/content/drive/MyDrive/VGG16/accuracy_{}.jpg".format(epoch))
        if epoch==50:
            plt.savefig("/content/drive/MyDrive/VGG16/accuracy_{}.jpg".format(epoch))
        if epoch==75:
            plt.savefig("/content/drive/MyDrive/VGG16/accuracy_{}.jpg".format(epoch))
